DPI Engine Setup Guide (Windows & macOS)
This guide will help you build and run the DPI Engine on Windows and macOS. Follow these steps exactly.
Option 1: Using Visual Studio (Recommended for Windows Beginners)
Step 1: Install Visual Studio
1. Download Visual Studio 2022 Community (free): • Go to: https://visualstudio.microsoft.com/downloads/ • Click "Free download" under Community
2. Run the installer
3. When asked "What workloads to install", select: • ✅ Desktop development with C++
...
1. If successful, you'll see dpi_engine.exe in the folder
Step 5: Run the Program
dpi_engine.exe test_dpi.pcap output.pcap

Option 2: Using MinGW-w64 (GCC for Windows)
Step 1: Install MSYS2
1. Download MSYS2 from: https://www.msys2.org/ • Click the download button for msys2-x86_64-xxxxxxxx.exe
2. Run the installer: • Install to C:\msys64 (default) • Keep "Run MSYS2 now" checked • Click Finish
3. A terminal window opens. Run these commands:
# Update package database
pacman -Syu

1. The window will close. Open "MSYS2 MINGW64" from the Start Menu (NOT the regular MSYS2!)
2. Install the compiler:
pacman -S mingw-w64-x86_64-gcc mingw-w64-x86_64-make

1. Type Y and press Enter when asked
Step 2: Add to PATH
...
2. Navigate to the project: cd C:\path\to\packet_analyzer
3. Build: g++ -std=c++17 -O2 -I include -o dpi_engine.exe ^     src/dpi_mt.cpp ^     src/pcap_reader.cpp ^     src/packet_parser.cpp ^     src/sni_extractor.cpp ^     src/types.cpp
...
4. After restart, Ubuntu will open automatically • Create a username and password when asked
...
Option 4: Using Visual Studio Code (Windows)
...
3. Search and install: • C/C++ (by Microsoft) • C/C++ Extension Pack (by Microsoft)
...
Option 5: Using macOS (Apple Silicon & Intel)
This works on both Apple Silicon (M1, M2, M3, M4) and Intel Macs.
Step 1: Install Xcode Command Line Tools
Open Terminal (Press Cmd + Space, type "Terminal", and press Enter) and run:
xcode-select --install

If it is already installed, you will see: xcode-select: error: command line tools are already installed.
Verify the compiler is active by checking its version:
clang++ --version

Step 2: Install Homebrew (Optional but Recommended)
Homebrew makes it easy to manage package dependencies if you need them later.
1. Run the installer script: /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
2. Crucial for Apple Silicon Macs: After installation finishes, copy and run the two setup lines provided in the terminal output under "Next steps" to add Homebrew to your environment. They look similar to this: echo 'eval "$(/opt/homebrew/bin/brew shellenv)"' >> ~/.zprofile eval "$(/opt/homebrew/bin/brew shellenv)"
3. Verify it works: brew --version
Step 3: Build the Project
1. Navigate to your project folder: cd /path/to/packet_analyzer  (Hint: You can type cd  and drag-and-drop the folder directly from Finder into the Terminal window)
2. Compile using Clang (macOS's native compiler): clang++ -std=c++17 -pthread -O2 -I include -o dpi_engine \     src/dpi_mt.cpp \     src/pcap_reader.cpp \     src/packet_parser.cpp \     src/sni_extractor.cpp \     src/types.cpp
3. If successful, an executable named dpi_engine will be generated in your current folder.
Step 4: Run the Program
Execute the binary with your network packet files:
./dpi_engine test_dpi.pcap output.pcap
...
2. Try a different output filename: dpi_engine.exe test_dpi.pcap result.pcap
Quick Reference
...
macOS (Clang)	clang++ -std=c++17 -pthread -O2 -I include -o dpi_engine src/dpi_mt.cpp src/pcap_reader.cpp src/packet_parser.cpp src/sni_extractor.cpp src/types.cpp
Run Commands
...
# With blocking
./dpi_engine input.pcap output.pcap --block-app YouTube --block-ip 192.168.1.50
...
8. Run DPI engine on it: dpi_engine.exe my_capture.pcap filtered.pcap
Need Help?
If you're stuck:
1. Make sure you followed EVERY step (don't skip any!)
2. Check the Troubleshooting section above
3. Try WSL (Option 3) or macOS native terminal—they are the most reliable environment variants.
